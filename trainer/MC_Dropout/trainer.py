    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.data_loader):
            ...
            for met in self.metric_ftns:
                self.train_metrics.update(met.__name__, met(output, target, type="Normal"))
            ...
            for batch_idx, (data, target) in enumerate(self.valid_data_loader):
                ...
                if samples == 1:
                    ...
                    for i in range(samples):
                        out = self.model(data, sample=True)
                        loss_i = self.criterion(out, target)
                        loss = loss + loss_i
                        outputs[:, :, i] = out
                ...
                for met in self.metric_ftns:
                    self.valid_metrics.update(met.__name__, met(outputs, target, type="MC_dropout"))
                # self.writer.add_image('input', make_grid(data.cpu(), nrow=8, normalize=True))
